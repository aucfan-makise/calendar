...
class CalendarFunction {
    ...
    private function insertSchedule($user_title, $user_detail, $user_start_time, $user_end_time) {
        if (strtotime ( $user_start_time ) >= strtotime ( $user_end_time )) {
            echo "<pre>登録時間がおかしいです。</pre>";
            return;
        }
    ...
    private function fetchSchedule($year, $month, $day) {
        try {
            ...
            $this->select_all_day_schedule_stmt->bindParam ( ":day_start_time", $day_start_time );
            $this->select_all_day_schedule_stmt->bindParam ( ":day_end_time", $day_end_time );
            $schedule = array ();
            if ($this->select_all_day_schedule_stmt->execute ()) {
                foreach ( $this->select_all_day_schedule_stmt as $row ) {
                    $schedule [$row ["my_schedules_id"]] = $row ["title"];
                }
            }
            
            // 開始だけかぶるやつ
            $this->select_start_schedule_stmt->bindParam ( ':day_start_time', $day_start_time );
            $this->select_start_schedule_stmt->bindParam ( ':day_end_time', $day_end_time );
            if ($this->select_start_schedule_stmt->execute ()) {
                foreach ( $this->select_start_schedule_stmt as $row ) {
                    $date = explode ( " ", $row ["start_time"] );
                    $time = explode ( ":", $date [1] );
                    $schedule [$row ["my_schedules_id"]] = $time [0] . ":" . $time [1] . "~ " . $row ["title"];
                }
            }
            
            // 終了だけかぶるやつ
            $this->select_end_schedule_stmt->bindParam ( 'day_start_time', $day_start_time );
            $this->select_end_schedule_stmt->bindParam ( ':day_end_time', $day_end_time );
            if ($this->select_end_schedule_stmt->execute ()) {
                foreach ( $this->select_end_schedule_stmt as $row ) {
                    $date = explode ( " ", $row ["end_time"] );
                    $time = explode ( ":", $date [1] );
                    $schedule [$row ["my_schedules_id"]] = "~" . $time [0] . ":" . $time [1] . " " . $row ["title"];
                }
            }
            ...
            // 1日で終わってしまう予定
            $this->select_day_schedule_stmt->bindParam ( 'day_start_time', $day_start_time );
            $this->select_day_schedule_stmt->bindParam ( ':day_end_time', $day_end_time );
            if ($this->select_day_schedule_stmt->execute ()) {
                foreach ( $this->select_day_schedule_stmt as $row ) {
                    $date = explode ( " ", $row ["start_time"] );
                    $start_time = explode ( ":", $date [1] );
                    $date = explode ( " ", $row ["end_time"] );
                    $end_time = explode ( ":", $date [1] );
                    $schedule [$row ["my_schedules_id"]] = $start_time [0] . ":" . $start_time [1] . "~" . $end_time [0] . ":" . $end_time [1] . " " . $row ["title"];
                }
            }
        } catch ( PDOException $e ) {
            echo "<pre> 予定の取得に失敗しました。:" . $e->getMessage () . "</pre>";
            die ();
        }
        return $schedule;
    }
     ...
     */
    public function __construct($selected_date, $calendar_size, $schedule_start_year, $schedule_start_month, $schedule_start_day, $schedule_start_hour, $schedule_start_minute, $schedule_end_year, $schedule_end_month, $schedule_end_day, $schedule_end_hour, $schedule_end_minute, $schedule_title, $schedule_detail) {
     ...
     * 表示するカレンダーの計算などを行う
     */
    private function initialize($schedule_start_date, $schedule_start_time, $schedule_end_date, $schedule_end_time, $schedule_title, $schedule_detail) {
        $this->connection = new PDO ( 'mysql:host=localhost;dbname=' . self::DATABASE_NAME, self::DATABASE_USER_NAME, "" );
        $this->select_all_day_schedule_stmt = $this->connection->prepare ( "SELECT my_schedules_id, title FROM my_schedules where start_time <= :day_start_time AND end_time >= :day_end_time" );
        $this->select_start_schedule_stmt = $this->connection->prepare ( "SELECT my_schedules_id, title, start_time FROM my_schedules where start_time >= :day_start_time AND end_time >= :day_end_time AND start_time <= :day_end_time" );
        $this->select_end_schedule_stmt = $this->connection->prepare ( "SELECT my_schedules_id, title, end_time FROM my_schedules where start_time <= :day_start_time AND end_time <= :day_end_time AND end_time >= :day_start_time" );
        $this->select_day_schedule_stmt = $this->connection->prepare ( "SELECT my_schedules_id, title, start_time, end_time FROM my_schedules where start_time >= :day_start_time AND end_time <= :day_end_time" );
        
        if (! empty ( $schedule_title )) {
            $date = implode ( "-", $schedule_start_date );
            $time = implode ( ":", $schedule_start_time );
            $start_time = $date . " " . $time;
            
            $date = implode ( "-", $schedule_end_date );
            $time = implode ( ":", $schedule_end_time );
            $end_time = $date . " " . $time;
